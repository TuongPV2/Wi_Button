...
void ready()
{
	...
	//debugf("Station. ip: %s mac: %s", WifiStation.getIP().toString().c_str(), WifiStation.getMAC().c_str());
}
...
void init()
{
	...
	//WifiStation.getMAC();
	//Serial.println(WifiStation.getMAC().toString());


	procTimer.initializeMs(5000, printoutRSSI).start(true);
}
